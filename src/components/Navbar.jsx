        ...
        <nav className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 transition-colors">
            <div className="flex items-center justify-between px-4 md:px-10 lg:px-40 py-3">
                ...
                <Link to="/" className="flex items-center gap-2 md:gap-3 group">
                    ...
                    <div className="leading-tight">
                        ...
                        <div className="hidden md:flex items-center gap-1.5 mt-0.5">
                            ...
                            <div className="text-[10px] text-gray-400 dark:text-gray-500 tracking-wide font-medium">TRUSTED · SECURE · SIMPLE</div>
                        </div>
                    </div>
                </Link>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <li key={item.path} className="relative group cursor-pointer">
                                <Link
                                    to={item.path}
                                    className={`transition-colors ${isActive ? 'text-blue-700 dark:text-blue-400' : 'hover:text-blue-700 dark:hover:text-blue-400'
                                        }`}
                                >
                                    {t(item.key)}
                                </Link>
                                <span
                                    className={`absolute -bottom-1 left-0 h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full transition-all ${isActive ? 'w-full' : 'w-0 group-hover:w-full'
                                        }`}
                                ></span>
                            </li>
                        )
                ...
                <div className="hidden lg:flex items-center gap-3">
                    ...
                    >
                        <span className={`fi ${languages[lang].flagClass} rounded-full w-6 h-6 object-cover`}></span>
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{languages[lang].label}</span>
                    </div>
                </div>
            ...
            {mobileMenuOpen && (
                <div className="lg:hidden bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 px-4 py-4 space-y-4">
                            ...
                            return (
                                <li key={item.path}>
                                    <Link
                                        ...
                                        className={`block px-3 py-2.5 rounded-lg text-[15px] font-medium transition-colors ${isActive
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                        ...
                        >
                            <span className={`fi ${languages[lang].flagClass} rounded-full w-5 h-5 object-cover`}></span>
                        </div>
                    </div>
                </div>
            )}
